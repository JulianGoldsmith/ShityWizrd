...
public class PhysicsObjectMaterial : ScriptableObject
{
    ...
    {
        float mass = myProperties.mass > 0 ? myProperties.mass : 1.0f;

        float hardness_factor = myProperties.hardness > 0 ? myProperties.hardness : 1f;
        if (otherProperties != null)
            hardness_factor *= (otherProperties.hardness > 0 ? otherProperties.hardness : 1f);

        float wall_or_floor_penalty = (otherProperties == null) ? 0.1f : 1.0f;

        return 150f * collision_impulse * hardness_factor * wall_or_floor_penalty *
            myProperties.brittleness * 0.1f / mass;
    }
    #endregion
