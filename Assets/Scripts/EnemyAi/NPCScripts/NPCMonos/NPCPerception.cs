...
public class NPCPerception : MonoBehaviour
{
    ...
    private void FixedUpdate()
    {
        ...
        if (currentTargetInSight != null)
        {
            ...
            {
                if (target != null)
                    Debug.DrawLine(root.transform.position + eyeOffset, currentTargetInSight.transform.position, Color.cyan);
            }
    ...
    {
        Collider[] targetsInRadius = Physics.OverlapSphere(root.transform.position, perceptionRadius, targetLayer);

        foreach (var targetCollider in targetsInRadius)
        {
            if (!targetCollider.CompareTag(targetTag)) continue;

            GameObject potentialTarget = targetCollider.transform.gameObject;
            if (potentialTarget == null) continue;

            Vector3 directionToTarget = (potentialTarget.transform.position - root.transform.position).normalized;

            if (Vector3.Angle(root.transform.forward, directionToTarget) < perceptionAngle / 2)
            {
                float distanceToTarget = Vector3.Distance(root.transform.position, potentialTarget.transform.position);
                Vector3 eyePosition = root.transform.position + eyeOffset;
    ...
    private void OnDrawGizmosSelected()
    {
        ...
        Gizmos.DrawWireSphere(root.transform.position, perceptionRadius);
        ...
        Gizmos.DrawLine(root.transform.position, root.transform.position + coneLeft);
        Gizmos.DrawLine(root.transform.position, root.transform.position + coneRight);
