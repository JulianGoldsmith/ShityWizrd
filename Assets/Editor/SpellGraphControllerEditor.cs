...
public class SpellGraphControllerEditor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        {
            if (!string.IsNullOrEmpty(saveSpellName)) controller.SaveSpellToAssets(saveSpellName);
            else Debug.LogError("Save Spell Name cannot be empty.");
        }
        ...
        {
            if (!string.IsNullOrEmpty(loadSpellName)) controller.LoadSpellByNameToCurrentItem(loadSpellName);
            else Debug.LogError("Load Spell Name cannot be empty.");
        }
